...
const createExoplanetsViz = () => {
  ...
  const background = container.append('image')
    .attr('id', 'background')
    .attr('xlink:href', bg)
    .attr('height', height)
    .attr('width', width)
  ...
  d3.csv(data, type).then(data => {
    ...
    stellerTempScale
      ...
      .range(['#FF0000', '#FFA500', '#FFFF00', '#FFFFFF', '#CCCCFF', '#6666FF', '#0000FF'])
  ...
  function getDimensions() {
    return {
      width: options.fullscreen ? document.documentElement.clientWidth : d3.select('#exoplanets-viz').node().clientWidth,
      height: options.fullscreen ? document.documentElement.clientHeight : d3.select('#exoplanets-viz').node().clientHeight,
    }
  }
  ...
  function draw(data) {
    ...
    const nodes = dataPoints.selectAll('circle')
      .data(data, d => d.pl_name)
      .join(
        enter => enter.append('circle')
        .attr('transform', d => 'translate(' + xt(d.pl_orbsmax) + ',' + 0 + ')')
        ...
        .style('stroke', d => !d.image ? stellerTempScale(d.st_teff) : 'none')
        .attr('r', 0)
        .on('mouseenter', d => {
        ...
        update => update.attr('transform', d => 'translate(' + xt(d.pl_orbsmax) + ',' + 0 + ')'),
    ...
    document.getElementsByClassName('btn-fullscreen')[0].onclick = fullscreen
    document.getElementsByClassName('cb-habitable')[0].onchange = showHabitableOnly

    function fullscreen() {
      options.fullscreen = !options.fullscreen
      document.getElementById('icon-fullscreen').classList.toggle('fa-compress-arrows-alt')
      document.getElementById('icon-fullscreen').classList.toggle('fa-arrows-alt')
      document.getElementById('exoplanets-viz').classList.toggle('fullscreen')
      draw(fullData)
    }
